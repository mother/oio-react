...
PopoverOpenSpecimen.description = 'The following example shows a <code>Popover</code> component that is open by default. Note that this situation is a little more complicated, because the <code>ref</code> that stores the button node is not immediately set during the first render (a DOM node must be rendered to the DOM before it can be referenced). In addition, changes to a ref do not trigger a re-render, so we use state to hold a referenece to the DOM element so that a re-render is triggered. So in this example, we make sure the <code>ref</code> is set before rendering <code>Popover</code> by using what\'s known as a "callback ref".'
